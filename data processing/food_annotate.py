...
for w in ["parchment", "wok", "shortening", "paper towel", "paper towels", "knead", "dust", "balls",
          "sauce", "soup", "loaf", "batter", "muffin cups", "stuffing", "cups", "cup", "dish", "glass",
          "glasses", "tin", "jar", "jars", "pan", "pans", "foil", "wrap", "paper", "dot"]:
...
# with open("annotated-samples500.json","r") as f:
    ...
    for i in range(len(samples)):
        ...
        samples[i] = samples[i].replace("\u00b0", " degrees")
        samples[i] = samples[i].replace("\t", " ")
        samples[i] = samples[i].replace("{ ", " {")
    ...
    # print(samples)
# print(samples)
samples_with_tags = [samples_corrected, tag_food]
import pandas
df = pandas.DataFrame()
df['samples'] = samples_corrected
df['tag'] = tag_food
df.to_csv('bert/food-anno-100000.csv', index=False)
